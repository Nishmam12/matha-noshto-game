...
static const Uint8 castle_left[42] = {
    120,117,115,113,111,109,107,106,105,104,103,102,102,101,101,100,100,101,
    100,100,100, 99, 99,100,100,101,100,101,102,102,103,104,105,105,106,107,
    108,109,110,112,114,117
};
static const Uint8 castle_right[42] = {
    125,129,131,133,135,137,139,140,141,141,142,142,142,143,143,142,142,143,
    143,143,142,142,142,143,143,142,141,141,140,140,139,138,137,136,136,135,
    134,133,132,130,129,127
};

static int castle_island_tile(int tx, int ty)
{
    int row;

    if (ty < CASTLE_RESERVE_Y0 || ty >= CASTLE_RESERVE_Y0 + CASTLE_RESERVE_H)
        return 0;
    row = ty - CASTLE_RESERVE_Y0;
...
static int castle_approach_path_tile(int tx, int ty)
{
    int path_y;
...
{
    return castle_island_tile(tx, ty) || castle_approach_tile(tx, ty)
        || castle_causeway_tile(tx, ty);
}
...
/* Replace the generated tiles in the Aetherhold footprint with a deterministic
 * coastal composition. This is intentionally applied before rivers/buildings:
 * those systems must see the island's real collision shape, while their random
 * decoration must never overwrite a handcrafted wall or approach. */
static void castle_apply_layout(World *w, int unlocked)
{
    int x, y;
    ...
    /* The approach is a fixed mainland shelf/road, not another island. */
    for (y = 0; y < OVERWORLD_H; y++)
        for (x = 0; x < WORLD_W; x++)
            if (castle_approach_tile(x, y)) {
                w->surf[y][x] = SURF_LAND;
                w->solid[y][x] = 0;
                w->bridge[y][x] = 0;
                w->path[y][x] = (Uint8)castle_approach_path_tile(x, y);
                w->sea_dist[y][x] = 0;
            }
...
static void place_rivers(World *w, Rng *rng, int *dist, int *queue)
{
    ...
    for (r = 0; r < RIVER_COUNT; r++) {
        ...
        for (tries = 0; tries < 300; tries++) {
            ...
            if (!is_castle_reserved(cx, cy) && dist[ci] >= RIVER_SRC_MIN
                && w->surf[cy][cx] != SURF_RIVER) {
                best = ci;
                break;
            }
        }
...
static void place_buildings(World *w, Rng *rng)
{
     ...
     * overworld's 144x80, which is exactly why the sampling range must be clamped
    ...
    for (tries = 0; tries < 5000 && w->bld_count < BUILDING_TARGET; tries++) {
        ...
        /* Aetherhold is a fixed layout. Random village footprints may not
         * enter its island, approach, or causeway. */
        {
            int rx, ry, reserved = 0;
            for (ry = by - 2; ry <= by + bh + 1 && !reserved; ry++)
                for (rx = bx - 2; rx <= bx + bw + 1 && !reserved; rx++)
                    if (is_castle_reserved(rx, ry))
                        reserved = 1;
            if (reserved)
                continue;
        }
 ...
 * game_init twelve times on one keypress, so the worst-case F2 hitch is
 ...
 *
 * The walk is `seed + 1`, deliberately the same step the R key takes, so a player pressing R past
 * a pathological seed and game_init stepping past it internally land on the same world. One rule,
 * not two. */
...
static int game_init(Game *g, Rngs *rngs)
{
     ...
     * exists to bound. Reusing this one frame costs nothing.
     *
...
#if WAYFARER_SELFTEST
        /* --genfail-test only: force the no-component condition so the recovery path has coverage.
         * Always 0 outside that test. See the flag's declaration.
         *
         * Placed HERE, immediately before the scan, and not next to world_gen where it reads more
         * naturally: castle_apply_layout, place_rivers and place_portal all carve open tiles, so an
         * all-solid grid written any earlier is partly undone before the scan sees it and the test
         * measures a healthy world while believing it forced a failure. Measured — the first version
         * of this hook sat after world_gen and every one of its three cases silently passed through
         * to a 16-region world. The condition being simulated is "the flood scan finds no open
         * overworld component", so it has to be true at the scan, not at generation. */
    ...
    if (biggest_first < 0) {
         ...
         * zero told entity_in_reach, shard_in_reach, mm_draw and render_grid's marker loop that all
         * 19 memories and all 8 shards were sitting in that corner. The whole codebase agrees that
         * -1 means "not placed" (Entity.tile, Game.shards, World.portal, World.well all use it);
         * this path was the one place that did not say it. */
        int i;
 ...
 * --decode-test's negative control has to hand the decoder a record that lies about its own
 * stream, and ART_SPRITES is const in .rodata. A local copy is legal where casting the const away
 * is not.
 *
                           ...
                           int fade, int flip)
{
     ...
     * checked this since it was written — it just never shipped.
     *
    ...
    while (i < n && y < sp->h) {
        ...
        for (k = 0; k < count && y < sp->h; k++) {
            ...
            /* SEC-1 clamp 3. art_palette fills pal[1 .. pal_n] only, so a larger index reads an
             * indeterminate stack value and paints a colour nobody chose. Index 0 is the bake's
             * guaranteed-transparent entry, so folding a bad index to 0 makes malformed data draw
             * a HOLE — visible, harmless, and reportable — rather than confetti. Same test
             * art_stream_ok_sp uses, so the decoder and the validator agree on what "in palette"
             * means. */
            if (v > sp->pal_n)
                v = 0;
            if (v) {
...
int main(int argc, char **argv)
{
    ...
    /* ERR-1: game_init may have walked to the next seed. `seed` is what the HUD and the window
     * title show, and the entire value of showing it is that --seed N reproduces the world on
     * screen. Resync or the number becomes a lie. */
    seed = rngs.seed;
