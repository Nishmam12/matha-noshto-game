    ...
    for (y = 0; y < WORLD_H; y++)
        for (x = 0; x < WORLD_W; x++) {
            ...
            if (w->bld_at[y][x]) {
                ...
                /* A channel cut into the ground, shallower than the sea so the
                 * two read as different bodies of water.
                 *
                 * FLAT, deliberately. Terracing the river so it steps down to
                 * the coast would make the tile rasteriser draw a water-coloured
                 * side face at each drop — a waterfall, essentially for free —
                 * but the `dist` field here is the chamfer distance INTO solid
                 * masses, which is 1 along the whole length of a one-tile-wide
                 * river. Getting a descent would mean carrying the
                 * distance-to-sea field out of place_rivers in a new per-tile
                 * array. Worth doing; not done here. See the phase file. */
                h = ELEV_WATER / 2;
            } else if (w->solid[y][x]) {
