 ...
 * back to the Mainland; Area 3 leads down to the Dungeon (Area 4). */
static int portal_dest(const Game *g)
{
    ...
    if (g->area == 2) return 1;
    if (g->area == 3) return area_complete(g) ? 4 : 0;
    return 0;
}
...
static int save_header_ok(const Uint8 *buf)
{
     ...
     *    unless the castle is complete (seven memories and all three souls)
     *    and the key was held.
     *  - SF_KING / SF_BEAST still need area 3 or area 4, and SF_BEAST still
     *    needs SF_KING.
     *
    ...
    if (area == 4) {
        if (mask_area_frags(restored, 3) < FRAGMENT_COUNT) return -1;
        if (mask_area_souls(restored, 3) < SOUL_COUNT) return -1;
        if (mask_area_frags(restored, 2) < FRAGMENT_COUNT) return -1;
        if (mask_area_souls(restored, 2) < SOUL_COUNT) return -1;
    }
    /* Later-area progress the gates could never have produced: Area 3
     * progress without the key is impossible on any honest path. Area 2
     * progress without the Area 1 threshold is likewise impossible. Earlier
     * areas' bits may be anything - return travel means a save in Area 1
     * can carry Area 2 progress and still be legitimate. */
    if (mask_area_frags(restored, 2) + mask_area_souls(restored, 2) > 0) {
        if (mask_area_frags(restored, 1) < PORTAL_MEMORY_THRESHOLD) return -1;
        if (mask_area_souls(restored, 1) < SOUL_COUNT) return -1;
    }
     ...
     * Area 1 threshold, Area 3 voices need the key. */
    {
         ...
         * she has been. The Dungeon's map is granted on arrival, so bit 3 is
         * only legal in Area 4. */
        if ((maps & 0x02u) && !area2_open) return -1;
        if ((maps & 0x04u) && !key) return -1;
        if ((maps & 0x08u) && area != 4) return -1;
        ...
        for (a = 1; a <= 3; a++) {
            ...
            for (k = 0; k < 6; k++) {
                ...
                props_build(LOGICAL_W, LOGICAL_H, &g->w, base, cam_x, cam_y,
                            g->ents, &g->p, (float)k * 0.2f, dl, -1, 0);
                for (j = 0; j < dl->n; j++) {
...
static int save_selftest(Uint64 base)
{
     ...
     * holds the last good load - bit-for-bit untouched. */
    {
        ...
        if (n != SAVE_SIZE) {
            ...
            struct { const char *name; Uint8 buf[SAVE_SIZE]; size_t len; } ctl[20];
            int nctl = 0;
