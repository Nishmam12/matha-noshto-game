...
**Current `.exe` size:** 1,091,584 bytes (on `feat/phase-14-bug-fixes` — Session 18 walk cycle)
**Suite status:** default gate **25/25 green in one run**, `--play-test --seeds 50` **50/50**, `--audio-test` PASS — all re-run from a fresh build in Session 18. `--land-test --seeds 500` **re-swept: fails exactly `85/417/430`**, identical to the pre-existing set at `428c9fd`; neither the character switch nor the walk cycle introduced a regression. `--fade-test` carries the `facing6` truth table with two non-sentinel controls plus the walk contract (4 properties, 2 controls); `--move-test` carries the held-standing-pose check. Each proven against a break.
**Current status:** **Walk cycle wired (Session 18): 143 sprites, six-way 8-frame stride driven by `Player.anim`, and standing holds the frame the stride stopped on.** The idle sheets are retired but kept on disk unbaked. **`WALK_FPS 12.0` is unjudged and needs a human to watch it run** — 16.0 restores the old cadence; the static standing pose needs the same judgement. Five of six facings remain unphotographed. Phases 00–12 all done; Phase 13 Aetherhold single-castle switched; invisible-wall + foliage + bridge/water bans + portal/enterprise thinning + cluster fog done pending visual screenshot. Phase 11
...
**Headroom:** 348,416 bytes under the 1,440,000 ship target (`382,976` under hard `1,474,560`) — delta `+6,144` over 1,085,440, of which only `+1,896` is the character; `4,285` is an untracked `bld_house_small_v4_frame_0.png` riding the globbed `buildings` category with no caller (Project Status P3.18)
...
- [[2026-08-12-session-02]] *(Session 18)* — **The walk cycle ships, and standing holds the last frame.** The bake was already broken on arrival: the six `idle_*.png` were deleted from the working tree while `$script:CharSheets` still named them, so any bake would have thrown. Measured the walk sheets rather than trusting [[Character Switch Plan]] §1a — `384×64` pitch 48 (identical geometry, no slicer work), **8 of 8 frames unique** in every sheet, diagonals splitting into two runs across 2–3 scanlines on frames `0/3/4/7`, `0` key-magenta hits. That is a real two-contact stride, which **reverses §D2's one-shared-cycle decision**. `$CharSheets` → six walk sheets at `Frames = 8`; `player_idle[6][7]` → `player_walk[6][8]`; `player_sprite_id` **drops its `clock` parameter** and indexes `p->anim`; `update_player`'s `anim = 0.0f` reset **deleted** (plus a one-cycle wrap), which is the whole mechanism behind the held standing pose. `WALK_FPS 12.0`. Release `1,091,584` (`+6,144`, of which only **`+1,896`** is the character — `4,285` is an untracked `bld_house_small_v4_frame_0.png` with no caller), `143` sprites, `376,571` const. `25/25` default, `--play-test 50/50`, `--land-test 500 = 85/417/430` unchanged. **Trap: the first `--move-test` check passed against a deliberately broken build** — 240 steps per direction is a whole multiple of the 40-step wrap, so the sweep leaves `anim` at exactly `0`, the same value the bug produces; rebuilt with coprime stepping and a live vacuity guard. **Owed:** the `12.0` cadence and the static standing pose both need a human; five of six facings unphotographed.
