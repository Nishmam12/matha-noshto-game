...
| cline (muse-spark-1.2-contributor) | TASK-01 character switch Groups A–D | `feat/phase-14-castle-full-set-dev-mode` | **Groups A–D DONE, E owed** — six-way idle `assets/player_new` `384×64` pitch 48 `0–6` (frame 7 dup skipped) via `CharSheets`+`ConvertTo-SpriteFromRegion` (same magenta strip/trim/anchor/RLE); `FACE6_*` + `facing6_from_intent(sx,sy)` pure screen intent `±X→down-diag`, `Player.facing6` render-only (save unchanged), `player_idle[6][7]` + `player_sprite_id(Player*,clock)` on `g->clock` `IDLE_FPS 8.0`; bake `110→152→136` (`370390` const `palette 11796/rle 356418/rec 2176`), old `player` unbaked (PNGs kept). `--sprite-test 136` + `--fade-test facing6 9 + control 6/9` green; sampled `--land/--play --seeds 20` PASS; release `1,085,440` (`+5,120` over 1,080,320, headroom 354,560). Group E visual gate owed: six facings standing+moving at `TILE 18`, `idle_down` face check, bob cadence, size read, full `25/25 --seeds 20` + `--land-test --seeds 500 =85/417/430` | 2026-08-12 | Written test-first per `.clinerules` (facing6 sweep inside `--fade-test` + naïve horiz-dominant negative control). Art plan [[Character Switch Plan]]; punch list `handoff/TASK-01-character-switch.md`. |

| Claude | TASK-01 review + 3 fixes, and the verification Session 16 was owed | `feat/phase-14-bug-fixes` | **DONE** — uncommitted | 2026-08-12 | Reviewed the row above, did not edit it. **Ran what it declared owed:** default gate **23/23**, `--play-test --seeds 50` **50/50**, `--audio-test` PASS (0.196 ms of 21.333 ms), `--land-test --seeds 500` fails **exactly 85/417/430** — no regression. **Three fixes, release +0 bytes (1,085,440):** (1) `facing6`'s negative control returned sentinels `999/998` on the two pure-horizontal cases, so the two checks pinning the *design decision* were caught for free — replaced with two plausible controls, an up-diagonal variant caught on **exactly 2** and the old dominant-axis shape caught on 6/9; (2) the `player_sprite_id` check compared two ids inside an `if` with an **empty body** and asserted nothing about D2 (`g->clock` not `p->anim`) — now asserts both halves; (3) `tools/bake.ps1` carried a 71-line line-for-line **duplicate** of `ConvertTo-Sprite` (`ConvertTo-SpriteFromRegion`) — collapsed into one function with optional region params, **proven behaviour-preserving: `art_data.h` byte-identical after re-bake**. Fixes 1 and 2 each proven against a throwaway break the old checks passed. Also dropped dead `WALK_FRAMES`/`WALK_FPS` + stale comment. **Visual gate: `idle_down` shows the face (trap cleared), reads legibly at `TILE 18`; the 8 fps cadence is still unjudged and needs a human.** `Player.facing`/`anim`/`FACE_*` are now dead but deliberately left — struct change, own pass. See [[2026-08-12-session-01]] Session 17. |
